...
void ListaContigua::insert(int posicao, Produto p) {
    if (!temEspaco()) {
        cout << "Erro: Lista cheia, não é possível inserir o produto.\n";
        return;
    }

    if (!indiceValido(posicao) && posicao != quant) {
        cout << "Erro: Índice inválido para inserção.\n";
        return;
    }

    // Move os elementos para frente a partir da posição especificada
    shiftEnd(posicao);
    
    // Insere o novo produto na posição desejada
    lista[posicao] = p;
    quant++;
}
